...
Grid_Points = input('Enter imax value either individually or as an array [7,12,22,42]: ');

for index = 1:length(Grid_Points)

imax = Grid_Points(index);
disp(imax)
jmax = imax;
...
if rem(imax,2) == 0
    ...
    plot(Uc,yc, c(index), 'LineWidth',1.5, 'DisplayName', sprintf('%d x %d', imax, jmax))
else
    Uc = u((imax+1)/2,:);
    plot(Uc,yc, c(index), 'LineWidth',1.5, 'DisplayName', sprintf('%d x %d', imax, jmax))
end
...
xlabel("U/U_{max}", "FontWeight","bold")

title(["Centreline Velocity, U vs y (x = 0.5) @ Re = " + num2str(Re)])

legend('-DynamicLegend');
%legend({strcat('b=',num2str(b(index)))},"FontSize",14)
hold on
...
if rem(imax,2) == 0
    ...
    plot(xc,Vc, c(index), 'LineWidth',1.5, 'DisplayName', sprintf('%d x %d', imax, jmax))
else
    Vc = v(:,(jmax+1)/2);
    plot(xc,Vc, c(index), 'LineWidth',1.5, 'DisplayName', sprintf('%d x %d', imax, jmax))
end

ylabel("V/U_{max}", "FontWeight","bold")
xlabel("X", "FontWeight","bold")
title(["Centreline Velocity, V vs x (y = 0.5) @ Re = " + num2str(Re)])
